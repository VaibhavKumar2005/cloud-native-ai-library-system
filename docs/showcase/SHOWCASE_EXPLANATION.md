...
* **CI/CD Pipeline**: We use GitHub Actions. Pushes run CI only, and Azure deployment is a separate manual promotion step. That gives us release control for demos while keeping the runtime Azure-native.
...
1. Run `.\scripts\demo\deploy_demo.ps1` in PowerShell. Ensure `GOOGLE_API_KEY` and `GROQ_API_KEY` are exported in your PowerShell first!
