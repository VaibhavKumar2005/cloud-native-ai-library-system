...
resource "azurerm_container_app" "backend" {
  ...
  template {
    ...
    container {
      ...
      cpu    = 0.5
      memory = "1Gi"
      env { name = "DEPLOY_MODE"; value = "cloud" }
      env { name = "ALLOWED_HOSTS"; value = "*" } # FIX 
      env { name = "POSTGRES_HOST"; value = azurerm_postgresql_flexible_server.pg.fqdn }
      env { name = "POSTGRES_USER"; secret_name = "pg-user" }
      env { name = "POSTGRES_PASSWORD"; secret_name = "pg-password" }
      env { name = "REDIS_URL"; secret_name = "redis-url" }
    }
    http_scale_rule { name = "http-scaler"; concurrent_requests = "10" }
  }

  secret { name = "pg-user"; value = var.pg_admin_user }
  secret { name = "pg-password"; value = var.pg_admin_password }
  secret { name = "redis-url"; value = "rediss://:${azurerm_redis_cache.redis.primary_access_key}@${azurerm_redis_cache.redis.hostname}:${azurerm_redis_cache.redis.ssl_port}/0" }
...
resource "azurerm_container_app" "frontend" {
  ...
  template {
    ...
    http_scale_rule { name = "http-scaler"; concurrent_requests = "20" }
  }
