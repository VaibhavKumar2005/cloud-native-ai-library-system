...
OTEL_EXPORTER_ENDPOINT = os.environ.get('OTEL_EXPORTER_ENDPOINT', '').strip()
...
def _set_span_attributes(span, attributes: Optional[Dict[str, Any]] = None):
    """Apply span attributes while preserving primitive types when possible."""
    ...
    try:
        ...
        if OTEL_EXPORTER_ENDPOINT:
            try:
                ...
                logger.info("OpenTelemetry using console exporter (OTLP exporter not available)")
        else:
            exporter = ConsoleSpanExporter()
            span_processor = SimpleSpanProcessor(exporter)
            logger.info("OpenTelemetry using console exporter (OTLP endpoint not configured)")
        ...
        try:
            ...
            logger.debug("Django auto-instrumentation skipped: %s", exc)

        try:
            from opentelemetry.instrumentation.requests import RequestsInstrumentor
            RequestsInstrumentor().instrument()
        except Exception as exc:
            logger.debug("Requests auto-instrumentation skipped: %s", exc)

        try:
            from opentelemetry.instrumentation.httpx import HTTPXClientInstrumentor
            HTTPXClientInstrumentor().instrument()
        except Exception as exc:
            logger.debug("HTTPX auto-instrumentation skipped: %s", exc)
    ...
    try:
        ...
        if current_span:
            ...
            for key, value in (attributes or {}).items():
                if value is None:
                    continue
                clean_attributes[key] = value if isinstance(value, (bool, int, float)) else str(value)
            current_span.add_event(name, attributes=clean_attributes)
    except Exception as e:
...
class OpenTelemetryMiddleware:
    ...
    def __call__(self, request):
        ...
        with tracer.start_as_current_span(span_name) as span:
            ...
            # Safely check user (may not be set for public endpoints like /api/health/)
