...
if ($LASTEXITCODE -ne 0 -or $vaultCheck -match "No value found") {
    $issues += "⚠️  Vault not initialized. Run: .\scripts\setup\init_vault.ps1"
} else {
...
if ($issues.Count -eq 0) {
    ...
    Write-Host "  1. .\scripts\setup\init_vault.ps1  (to populate Vault with API keys)" -ForegroundColor White
