...
$TEST_PDF = Join-Path $PROJECT_ROOT "docs\\assets\\demo\\test-document.pdf"
...
if ($LASTEXITCODE -ne 0 -or $vaultCheck -match "No value found") {
    ...
    Write-Host "     Fix: Run .\scripts\setup\init_vault.ps1 to add API keys" -ForegroundColor Yellow
