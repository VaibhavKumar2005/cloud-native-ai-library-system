...
if (-not $vaultHealthy) {
    Write-Host "  ⚠️  Vault is sealed or uninitialized. Run: ./init_vault.ps1" -ForegroundColor Yellow
}
...
if (-not $pgvectorHealthy) {
    Write-Host "  ⚠️  pgvector not enabled. Run: python backend/setup_pgvector.py" -ForegroundColor Yellow
}
...
if ($allHealthy) {
    ...
    Write-Host ""
    Write-Host "  📚 See DEMO_GUIDE.md for complete demo flow" -ForegroundColor Cyan
} else {
    ...
    Write-Host "    4. See DEMO_GUIDE.md troubleshooting section" -ForegroundColor Gray
}
Write-Host "═══════════════════════════════════════════════════════════════" -ForegroundColor Cyan
Write-Host ""
