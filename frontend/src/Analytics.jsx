...
const Analytics = () => {
  ...
  const documentMetrics = {
    ...
    processingRate: documents.length > 0 
      ? (documents.filter(d => d.processed).length / documents.length) * 100 
      : 0
  };
      ...
      <nav className="sticky top-0 z-50 border-b border-slate-800 bg-slate-950/80 backdrop-blur-md">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <div className="rounded-lg bg-violet-500/10 p-2 border border-violet-500/20">
              <BarChart3 className="h-5 w-5 text-violet-400" />
            </div>
            <div>
              <h1 className="text-lg font-bold tracking-tight">Analytics Dashboard</h1>
              <p className="text-[10px] font-mono uppercase tracking-widest text-slate-500">VeriRAG Intelligence Hub</p>
            </div>
          </div>
